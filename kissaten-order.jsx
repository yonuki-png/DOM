...
export default function App() {
  ...
  const customChange = !isNaN(customPaid) && customPaid >= total ? calcChange(total, customPaid) : null;
            ...
            {filtered.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <div style={styles.menuPrice}>¥{item.price.toLocaleString()}</div>
                  <div style={styles.qtyRow}>
                    ...
                    <button style={styles.qtyBtn} onClick={() => addItem(item.id)}>
                      ＋
                    </button>
                  </div>
                </div>
          ...
          {totalCount > 0 && (
            <div style={styles.bottomBar}>
              <div style={styles.bottomTotal}>
                合計 <span style={styles.bottomTotalNum}>¥{total.toLocaleString()}</span>
              </div>
        ...
        <div style={styles.body}>
          <div style={styles.receipt}>
            ...
            <div style={styles.receiptTotal}>
              <span>合 計</span>
              <span style={styles.receiptTotalNum}>¥{total.toLocaleString()}</span>
            </div>
          </div>
          ...
          {total > 0 && (
            <>
                ...
                {suggestions.map((paid) => {
                  ...
                  return (
                    <div key={paid} style={styles.suggestCard}>
                      <div style={styles.suggestPaid}>¥{paid.toLocaleString()} を渡す</div>
              ...
              {!isNaN(customPaid) && customPaid < total && customInput !== "" && (
                <div style={styles.errorMsg}>金額が足りません</div>
              )}
            </>
          )}

          <button style={styles.clearBtn} onClick={() => { clearOrder(); setView("menu"); }}>
            🗑 注文をリセット
          </button>
        </div>
      )}
